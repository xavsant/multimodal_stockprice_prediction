...
# # Average the embeddings to create a single representation of the day's news without making the input too large
...
#             # If there are multiple articles for the same day, average the embeddings
#             if dates[i] in text_embeddings:
#                 text_embeddings[dates[i]].append(embedding)
#             else:
...
#     # Now average the embeddings for each day
#     logger.info("Averaging the embeddings for each day...")
#     for date in text_embeddings:
#         text_embeddings[date] = np.mean(text_embeddings[date], axis=0).tolist()  # Averaging and converting to list for JSON compatibility
