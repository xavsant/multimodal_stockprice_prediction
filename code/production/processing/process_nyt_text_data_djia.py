...
class process_text_data:
    '''
    Process NYT article data retrieved using 'retrieve_nyt_metadata_djia.py' for specific sections
    ...
    def __init__(self, params: dict):
        ...
        self.company = params['company'] # fixed to ^DJI as company instead
    ...
    def process(self, sections):
        ...
        Note: sections refer to the desired section names of the articles returned
        '''
        article_df = self.__read_all_articles()
        cleaned_df = self.__clean_data(article_df, sections)
    ...
    def __read_articles_in_file(self, data):
        ...
        for article in data:
            ...
            rank = next( # this is not relevant for djia
                (
                    item['rank']
    ...
    def __clean_data(self, df, sections):
        cleaned_df = df.copy()
        ...
        # Remove articles that are not in relevant sections
        cleaned_df = cleaned_df[cleaned_df['section_name'].isin(sections)]
